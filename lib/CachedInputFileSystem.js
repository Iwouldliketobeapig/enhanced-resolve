...
class Storage {
	...
	// 向data中set值,更新this.count,执行ensureTick,执行name中的函数
	...
	// 向data中set值，更新this.count,执行ensureTick
	...
	// name存在于running则push(callback),!=> name存在于this.data则返回process.nextTick;!=>running set name和执行provider
	provide(name, provider, callback) {
		...
		if(this.duration > 0) {
			...
			if(data) {
				return process.nextTick(() => { // ？？？ process在此处没有定义
